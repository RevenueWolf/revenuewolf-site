        ...
        {pillars.map((p) => {
          ...
          return (
            <button key={p.id} onClick={() => setActive(isActive ? null : p.id)}
              ...
              <span style={{ fontSize: 12, fontFamily: "'JetBrains Mono', monospace", letterSpacing: 2, color: isActive ? "#E63946" : "#1a1a1f", fontWeight: 800, textTransform: "uppercase", transition: "color 0.3s" }}>{p.label}</span>
            </button>
          );
        })}
      </div>
...
function DetailCard({ pillar }) {
  if (!pillar) return (
    <div style={{ textAlign: "center", padding: "36px 24px", color: "#8a8a95", fontFamily: "'JetBrains Mono', monospace", fontSize: 14, letterSpacing: 1.5, minHeight: 110, fontWeight: 500 }}>
  ...
  return (
    <div style={{ padding: "32px 36px", background: "#fff", border: "2px solid rgba(230,57,70,0.2)", borderRadius: 6, minHeight: 110, maxWidth: 680, margin: "0 auto", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", animation: "fadeIn 0.3s ease" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
        ...
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 15, letterSpacing: 3, color: "#E63946", fontWeight: 800, textTransform: "uppercase" }}>GTM {p.label}</span>
      </div>
      <p style={{ fontFamily: "'Source Serif 4', Georgia, serif", fontSize: 20, lineHeight: 1.6, color: "#1a1a1f", margin: "0 0 12px 0", fontWeight: 600 }}>{p.tagline}</p>
      <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, color: "#C1272D", margin: 0, letterSpacing: 0.5, fontWeight: 500, opacity: 0.75 }}>{p.missing}</p>
    </div>
  );
}
...
function AssessmentForm() {
  ...
  const inputStyle = { width: "100%", padding: "14px 18px", fontFamily: "'Source Serif 4', Georgia, serif", fontSize: 16, color: "#1a1a1f", background: "#fff", border: "2px solid #e2e0dc", borderRadius: 4, outline: "none", transition: "all 0.25s", boxSizing: "border-box" };
  const labelStyle = { display: "block", fontFamily: "'JetBrains Mono', monospace", fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: "#8a8a95", marginBottom: 8, fontWeight: 600 };
  const btnPrimary = { fontFamily: "'JetBrains Mono', monospace", fontSize: 13, letterSpacing: 3, textTransform: "uppercase", background: "#E63946", border: "none", color: "#fff", padding: "16px 40px", cursor: "pointer", borderRadius: 3, fontWeight: 800 };
  const btnGhost = { fontFamily: "'JetBrains Mono', monospace", fontSize: 11, letterSpacing: 2.5, textTransform: "uppercase", background: "none", border: "2px solid #e2e0dc", color: "#8a8a95", padding: "12px 28px", cursor: "pointer", borderRadius: 3, fontWeight: 700, marginRight: 12 };
  ...
  if (step === "capture") return (
    <div style={{ animation: "fadeIn 0.4s ease" }}>
      <div style={{ textAlign: "center", marginBottom: 32 }}>
        ...
        <p style={{ fontFamily: "'Source Serif 4', Georgia, serif", fontSize: 18, color: "#4a4a55", maxWidth: 420, margin: "0 auto", lineHeight: 1.6 }}>Tell us who you are so we can come prepared with ideas specific to your gaps.</p>
      </div>
  ...
  if (step === "results") {
    ...
    return (
      <div style={{ textAlign: "center", padding: "56px 24px" }}>
        ...
        <div className="gtm-score" style={{ fontSize: 72, fontFamily: "'JetBrains Mono', monospace", fontWeight: 800, lineHeight: 1, marginBottom: 18, color: verdict.color }}>
          ...
          {assessmentQuestions.map((q, i) => {
            ...
            return (
              <div key={i} className="bar-row" style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 10 }}>
                <span className="bar-label" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, letterSpacing: 2, color: "#8a8a95", width: 110, textAlign: "right", textTransform: "uppercase", fontWeight: 600 }}>{q.pillar}</span>
      ...
      {assessmentQuestions.map((q, qi) => (
        <div key={qi} style={{ marginBottom: 32, paddingBottom: 32, borderBottom: qi < total - 1 ? "2px solid #e2e0dc" : "none" }}>
          <div className="question-row" style={{ display: "flex", gap: 14, marginBottom: 16 }}>
            <span className="question-pillar" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, letterSpacing: 2.5, color: "#E63946", fontWeight: 700, textTransform: "uppercase", minWidth: 110, paddingTop: 3 }}>{q.pillar}</span>
...
export default function GTMFramework() {
  ...
  return (
    <div style={{ background: "#F8F7F5", fontFamily: "'Source Serif 4', Georgia, serif", color: "#1a1a1f" }}>
      <style>{`
          ...
          <div className="mobile-pillars">
            {pillars.map((p) => (
              <button key={p.id} onClick={() => setActive(active === p.id ? null : p.id)}
                ...
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, fontWeight: 800, letterSpacing: 1.5, textTransform: "uppercase", color: active === p.id ? "#E63946" : "#1a1a1f" }}>{p.label}</div>
              </button>
            ))}
          </div>
        </div>
